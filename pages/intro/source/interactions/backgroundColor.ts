...
export default function valBackgroundColor({ DOM, interActionProp, progressRatio }: cssUpdateProps) {
    ...
    const color = [];
    for (let i = 0; i < 4; i++) {
        let RGBAvalue = calCssValue({startRatio, endRatio, progressRatio, value:[ prevRGBA[i], nextRGBA[i]]});
