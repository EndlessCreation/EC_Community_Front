...
const identities = [
  {
    icon: <GrowthIcon width="100%" height="100%" className="icon" />,
    title: 'Growth',
    description: 'EC는 동아리원 모두의 성장을 격려하고 응원합니다.',
  },
  {
    icon: <SharingIcon width="100%" height="100%" className="icon" />,
    title: 'Sharing',
    description: 'EC는 개인의 지식과 경험을 공유하는 장이 되어줍니다.',
  },
  {
    icon: <CooperationIcon width="100%" height="100%" className="icon" />,
    title: 'Cooperation',
    description: 'EC는 더 나은 학습 환경을 위해 함께 고민하고 협력합니다.',
  },
];
        ...
        <Content>
          ...
          <Box className="container">
            {identities.map((identity, index) => (
              <ScrollRevealSlideAnimation startPosition={index + 1}>
                <Box className="symbol">
                  {identity.icon}
                  <Text className="iconTitle">{identity.title}</Text>
                  <Text className="description">{identity.description}</Text>
                </Box>
        ...
        <Divider>
          ...
          <ScrollRevealRender>
            <Box className="content">
              <CountUp
                duration={1.7}
                end={clubData.activeMemberCount}
                className="title"
                suffix="명"
              />
